...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Build CSS custom properties for chart theming.
  // This is safe: id and config keys are developer-controlled, not user input.
...
`
  ...
  return (
    // eslint-disable-next-line react/no-danger -- safe: CSS generated from developer-controlled chart config, not user input
                ...
                >
                  {formatter && item?.value !== undefined && item.name ? (
                    formatter(
                      ...
                      index,
                      payload as Parameters<typeof formatter>[4]
                    )
                  ) : (
                    <>
                      {itemConfig?.icon ? (
